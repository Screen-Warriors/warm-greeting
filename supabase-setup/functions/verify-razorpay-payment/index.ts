...
// Secrets required: RAZORPAY_KEY_SECRET, RESEND_API_KEY (optional but
// recommended), RESEND_FROM_EMAIL (optional), OWNER_NOTIFY_EMAIL (optional),
// SITE_URL (optional — used for tracking link in emails).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const expected = await hmacSha256Hex(`${b.razorpay_order_id}|${b.razorpay_payment_id}`, secret);
    ...
    try {
      ...
      if (rpcErr) console.error("decrement_stock returned:", rpcErr);
    } catch (e) {
